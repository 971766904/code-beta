...
class freqdomain:
    ...
    def psd_features(self):
        ...
        mpf, fmax, fmin,  Ptotal = psd.psd2(data, fs=self.myfs)  # 注意引用方式
    ...
    def main_freq(self, percent1=0.3, percent2=0.5, percent3=0.9):
        mpf, fmax, fmin,  Ptotal = self.psd_features()
        mean1, max1, min1, var1, mean2, max2, min2, var2, mean3, max3, min3, var3 = self.SEF(percent1=percent1,
                                                                                             percent2=percent2,
                                                                                             percent3=percent3)
        freq_feature = [mpf, fmax, fmin, Ptotal, mean1, max1, min1, var1, mean2, max2, min2, var2, mean3, max3,
                        min3, var3]
