...
def lgb_cv(num_leaves, min_data_in_leaf, bagging_fraction, feature_fraction, lambda_l1,  lambda_l2, max_depth, data, target, feature_name, categorical_feature):
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(data, target)):
        ...
        clf = lgb.train(param, trn_data, 300, valid_sets=[trn_data, val_data], verbose_eval=100, early_stopping_rounds=30,feval=lgb_f1_score,)
        oof[val_idx] = clf.predict(data[val_idx], num_iteration=clf.best_iteration)
        del clf, trn_idx, val_idx
        gc.collect()
    return f1_score(target, np.round(oof))
...
def optimize_lgb(data, target, feature_name='auto', categorical_feature='auto'):
    def lgb_crossval(num_leaves, min_data_in_leaf, bagging_fraction, feature_fraction, lambda_l1, lambda_l2, max_depth):
        return lgb_cv(num_leaves, min_data_in_leaf, bagging_fraction, feature_fraction, lambda_l1, lambda_l2, max_depth, data, target, feature_name, categorical_feature)
    ...
    start_time = timer()
    optimizer.maximize(init_points=5, n_iter=10, acq='ucb', kappa=10)
    timer(start_time)
